...
def P0(X):
    return X ^ _left_rot(X, 9, 32) ^ _left_rot(X, 17, 32)

def P1(X):
    return X ^ _left_rot(X, 15, 32) ^ _left_rot(X, 23, 32)
...
def sm3_extend(m):
    ...
    for i in range(16, 68):
        w1.append(P1(w1[i - 16] ^ w1[i - 9] ^ _left_rot(w1[i - 3], 15, 32)) ^ _left_rot(w1[i - 13], 7, 32) ^ w1[i - 6])
    for i in range(64):
...
def sm3_compress(m, reg):
    ...
    A, B, C, D, E, F, G, H = reg
    for j in range(64):
        ss1 = _left_rot((_left_rot(A, 12, 32) + E + _left_rot(T[j], j, 32)) & 0xffffffff, 7, 32)
        ss2 = ss1 ^ _left_rot(A, 12, 32)
        tt1 = (FF(j, A, B, C) + D + ss2 + w2[j]) & 0xffffffff
        tt2 = (GG(j, E, F, G) + H + ss1 + w1[j]) & 0xffffffff
        A, B, C, D, E, F, G, H = tt1, A, _left_rot(B, 9, 32), C, P0(tt2), E, _left_rot(F, 19, 32), G
    reg[0] ^= A
    reg[1] ^= B
    reg[2] ^= C
    reg[3] ^= D
...
if __name__ == "__main__":
    ...
    print (sm3_calc(b'flag{just_for_fun}'))